...
| dataDir | string   | Location of Safari's user data directory. If not set, the default location will be used.<ul><li>OS X: <code>/Users/$USER/Library/Safari</code></li><li>Windows: <code>%APPDATA%\Apple Computer\Safari</code></li></ul> |
| extensions | array    | The extensions that should be installed when starting Safari. Each extension should be specified as the base64 encoding of a ` .safariextz ` file. |
| skipExtensionInstallation | boolean  | Whether to skip installing the SafariDriver extension (an extension must be pre-installed; this is intended for working directly on the SafariDriver). |
